...
- Configure custom CSS (line 22 - xx)
...
sanitation <- WDI(country = "all", indicator = "SH.STA.BASS.ZS", start = 1960,
...
# inspect NA rows for non-SH.STA.BASS.ZS columns. TAKEAWAY: among the 180 cases that have NA values, there are three "countries" (under the country column): "Sub-Saharan Africa", "Latin America & Caribbean", and "North Macedonia". For "Sub-Saharan Africa" and "Latin America & Caribbean", I could later filter them out, as they should have "Aggregates" under the region column, but instead they are having NAs. I think this may be a mistake in the server. For "North Macedonia", I believe it should be a mistake to have NA under the region column (should be "Europe & Central Asia"), and NA under the income column (should be "Upper middle income", based on the 2019 income level defintion in the "CLASS.xls" file (http://databank.worldbank.org/data/download/site-content/CLASS.xls) downloaded on June 19 2020 from this page (https://datahelpdesk.worldbank.org/knowledgebase/articles/906519-world-bank-country-and-lending-groups)). I will need to update the NA values for North Macedonia later.
sanitation %>% 
  filter(iso3c %in% NA |region %in% NA |capital %in% NA |longitude %in% NA |latitude %in% NA | income %in% NA | lending %in% NA ) %>% 
  View()

# inspect NA rows for the SH.STA.BASS.ZS column. TAKEAWAY: these NA rows means there are no data in those years, which makes sense. Nothing jumps out, will filter these NA rows out later
sanitation %>% 
...
sanitationProcessed <- sanitation %>% 
  ...
  # fix the NA columns for North Macedonia, more details in line 81
  mutate(income = ifelse(country %in% "North Macedonia", "Upper middle income", income)) %>%
  # drop rows whose region is Aggregates, or NA ("Sub-Saharan Africa", "Latin America & Caribbean"), more details in line 81
...
tf <- tween_states(sanitationProcessedList, tweenlength= 6, statelength=0, ease='linear',nframes=200)
...
tf2 <- tween_states(yearDataList, tweenlength= 6, statelength=0, ease='exponential-in-out',nframes=200)
...
saveGIF({
  ...
  for (i in 1:max(tf$.frame)) {
    ...
    # define the state that I want the plot to "pause" (in this case, the last frame)
    pause_frames <- c(max(tf$.frame))
    
    # for each frame expect for the first, start by drawing a blank new page on the screen
    if (i != 1) {
      grid.newpage()
    }
    
    # then draw with background color
    grid.draw(rectGrob(gp=gpar(fill="#e0eaef", lwd = 1)))
...
As the year progresses, the circles move along the X-axis. The color of the circle indicates whether the indicator in that country has decreased (red), stayed the same (orange), or increased (green) from 2000 to 2017. As the circles move, the distribution of countries reshapes.
...
Further analysis could investigate other non-income factors that may explain the gap in the share of the population using at least basic sanitation services. Case studies of the outlier countries that have the largest increase or decrease would reveal insights on what is needed to improve sanitation situation around the world.


## <span>Source and method</span>
The 191 countries represented in the chart are the countries that have data for all years between 2000 and 2017. The list of countires and more information could be seen below. The term country does not imply political independence but refers to any territory for which authorities report separate social or economic statistics.
